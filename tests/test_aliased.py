...
def test_groups():
    ...
    test = {
        ...
        'c': aliased.Group('c', 'C'),
        'C': aliased.Group('c', 'C'),
        'd0': aliased.Group('d0', 'd1', 'd2'),
        'd1': aliased.Group('d0', 'd1', 'd2'),
        'd2': aliased.Group('d0', 'd1', 'd2'),
    }
...
def test_groups_set_operations():
    ...
    for f, (x, y, r) in cases.items():
        ...
        assert f(gx, gy) == aliased.Groups(*r)
...
def update_groups(
    ...
    *others,
    ...
    for old, new in modified.items():
        keys = new or old
        for key in keys:
            assert groups.get(key) == aliased.Group(*keys)
...
def merge_groups(
    ...
    *others,
    ...
    for old, new in modified.items():
        keys = new or old
        for key in keys:
            assert merged.get(key) == aliased.Group(*keys)
        for key in old:
            assert groups.get(key) == aliased.Group(*old)
    for keys in inserted:
